...
----
 ...
 - Предоставить встроенную поддержку хранения длинных значений (чанкование) внутри
   libmdbx: автоматическое дробление значения на чанки при записи и прозрачная
   сборка при чтении (ключ расширяется суффиксом с номером чанка). Сейчас это
   делается на стороне приложения (см. AGENTS-GUIDE.md §1.3 и memory-mdbx-server.py
   B48); перенос в ядро устранит дублирование логики у всех клиентов и учтёт лимиты
   mdbx_env_get_maxkeysize()/mdbx_env_get_maxvalsize() автоматически. Ограничение
   dupsort: значения хранятся как ключи вложенного b-tree, поэтому их максимальный
   размер ~ половина страницы (не overflow-цепочка); для произвольных больших
   значений нужно либо использовать не-dupsort таблицу, либо чанкование.
