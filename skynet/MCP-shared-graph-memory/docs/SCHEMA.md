...
- **модуль** — из контролируемого словаря (см. §2.4 «Домены знаний»);
- **тема** — 1–3 нормализованных слова через дефис, из словаря, ≤ 30 символов.

### 2.3 Домены знаний

Знания делятся на домены; **домен кодируется модулем** ключа (схема ключа
остаётся `тип:модуль:тема`, новый сегмент не вводится). Это позволяет запросам
роя и общим практикам сосуществовать: выборка по домену = выборка по модулю.

| Домен | Модули | Содержимое |
|---|---|---|
| **0. meta** (нулевой, обязательный) | `meta` | знания о знаниях: карта, схема, процедуры, самонаблюдения |
| **1. general** | `practice` | общие практики и принципы (Кайдзен, методологии) |
| **2. swarm** | `swarm`, `coordination` | рой и протоколы (доки — в skynet-draft) |
| **3. project** | `crypto`, `network`, `storage`, `ui`, `build`, `testing`, `review`, `platform`, `core`, `memory` | проект libmdbx |
| **4. todo** | `todo` | неклассифицированное, ждёт разбора |
| **5. archive** | `archive` | устаревшее/legacy (хранится, не импортируется) |

Модули доменов 0–5 входят в `DEFAULT_MODULES` (`normalize.py`) и доступны сразу.
Карта доменов хранится в записи `fact:meta:knowledge-map` (самописание памяти).
