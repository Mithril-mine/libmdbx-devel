...
- Подробно о реализации и о судьбе таблиц, созданных/удалённых во вложенной транзакции, — раздел 14.8.
...
- **Политика переиспользования** выбирается флагом среды: по умолчанию — **FIFO** (в первую очередь
  переиспользуются страницы, освобождённые раньше всех); флаг `MDBX_LIFORECLAIM` включает **LIFO**
  (самые свежие освобождения). Механика, преимущества и сложности — в разделе 14.1.
...
Подробно о том, какую проблему решает BigFoot и чем платим, — раздел 14.2.
...
Этот механизм подробно разобран в ответе на вопрос №1 (раздел 17.1).
...
- При **парковке** читательской транзакции слот помечается специальным псевдо-идентификатором и
  ...
  и должен перезапустить чтение. Подробно о парковке и вытеснении — раздел 14.9.
...
Подробный разбор — в ответе на вопрос №2 (раздел 17.2).
О механике роста/сжатия файла «на ходу» и о бесплатной авто-компактификации — разделы 14.10–14.11.
...
- **Удаление**: CoW листа → удалить узел → при опустошении ниже порога — **rebalance/merge** с
  ...
  против «равномерности заполнения» — это прямой рычаг WAF, раздел 17.4).
- **Массовое удаление** целых диапазонов «гроздьями» (вырезание страниц и ветвей) — раздел 14.5.
...
- Подробно об устройстве и применении dupsort-таблиц — раздел 14.3.
...
времени и целям свободного пространства. Подробно — раздел 14.12.
...
выходом. Подробно — в ответе на вопрос №3 (раздел 17.3).
...
## 14. Глубокое погружение: избранные механизмы

> Раздел собран по принципу «от простого к сложному»: каждая тема опирается на предыдущие
> и на материал разделов 2–13. Цель — дать рабочее, а не справочное понимание: как устроено,
> какие проблемы решает и чем за это платит.

### 14.1. LIFO-переиспользование: как работает, преимущества и сложности

**Как работает.** Освобождённые страницы попадают в GC-записи, ключ которых — номер транзакции,
освободившей их (раздел 6.1). Переиспользованию подлежат записи с ключом ≤ детента (раздел 6.2).
Порядок выбора страниц внутри этого пула задаётся политикой:

- **FIFO (по умолчанию)** — берутся страницы из самой старой подходящей записи: «первым
  освободился — первым вернулся». Список освобождённых страниц живёт «долго», дольше оставаясь
  на диске холодным.
- **LIFO (`MDBX_LIFORECLAIM`)** — берутся страницы из самой свежей подходящей записи: «последним
  освободился — первым вернулся». Круг циркуляции страниц становится **минимально коротким**.

**Преимущества LIFO.** Страницы, освобождённые только что, с высокой вероятностью ещё находятся в
оперативной памяти и в кэше записи (write-back cache) диска. Возвращая именно их в оборот, мы:
- минимизируем число страниц, перезаписываемых в памяти и на диске в серии записывающих
  транзакций — рабочий набор держится «тёплым»;
- создаём идеальные условия для дискового write-back кэша, который может объединять записи одной
  и той же страницы.

На системах с дисковым write-back кэшем это способно увеличить производительность записи в разы.

**Сложности и ограничения.**
- LIFO почти не даёт выигрыша в сочетании с `MDBX_SAFE_NOSYNC`/`MDBX_UTTERLY_NOSYNC`: там
  переиспользование и так ограничено зоной до последнего steady-коммита, и длину цикла определяет
  частота `env_sync()`, а не выбор FIFO/LIFO.
- LIFO усиливает «горячность» небольшого набора страниц: повышается неравномерность, что нужно
  учитывать при оценке WAF и локальности.
- Порядок PNL внутри записей фиксирован (номера по убыванию); выбор политики влияет на то, из
  каких записей и в каком порядке сливаются списки при поиске последовательностей (раздел 17.1).

### 14.2. BigFoot: какая проблема и как она решается

**Проблема.** Одна запись GC физически ограничена: она должна помещаться в одну страницу и вмещает
не более `MAX_GC1OVPAGE ≈ 1000` номеров страниц (~4 МБ покрытия при странице 4 КБ). Если одна
транзакция освобождает больше страниц, чем влезает в одну запись, «обычный» формат записи ломается:
некуда писать сотни тысяч номеров страниц одного огромного значения.

**Решение.** Для таких освобождений запись GC «размазывается» в **цепочку записей на
последовательных номерах транзакций** — это и есть BigFoot. Номер транзакции цепочки увеличивается,
«эффективный» номер освобождения становится последним в цепочке, а читатели обязаны быть новее всей
цепочки, чтобы эти страницы можно было переиспользовать.

**Что это даёт.**
- Единый формат записей сохраняется: нет записи «неограниченного» размера;
- очень большие значения (вплоть до `MDBX_MAXDATASIZE ≈ 2 ГБ`) обслуживаются без специальных
  исключений в GC-дереве;
- механизм повторно используется и для массовых удалений, и для дефрагментации.

**Чем платим.** Цепочка означает больше записей в GC-дереве → выше его высота → глубже поиск и
дороже коммитная обработка GC (подробная цепочка — в ответе на вопрос №1, раздел 17.1).
Поэтому у коммита есть ограничители стоимости (раздел 6.4): при переполнении лимитов проще дописать
страницы в хвост файла, чем продолжать копить цепочку.

### 14.3. dupsort-таблицы: устройство, преимущества, применение

**Устройство.** Флаг `MDBX_DUPSORT` превращает таблицу в multimap: с одним ключом связано
упорядоченное множество значений. Значение в такой таблице играет роль «второго ключа», и для него
применяются те же правила сортировки (можно комбинировать с `MDBX_REVERSEDUP`, `MDBX_INTEGERDUP`,
`MDBX_DUPFIXED`). Хранение выбирается по количеству значений на ключ:
- немного значений — плотная **суб-страница** (вложенная страница внутри листа), параметризуется
  опциями суб-страниц;
- много значений — отдельное **вложенное B+tree** (nested tree);
- фиксированный размер значений — специализированные **dupfix-страницы** плотной упаковки.

Значение-«мультивылью» не может уходить на overflow-страницы, поэтому ограничено рамками ключа
(порядка половины страницы). Глубина вложенности для заданного ключа доступна через
`mdbx_dbi_dupsort_depthmask`.

**Преимущества.** Одной таблицей выражается связь «один ко многим» без отдельных таблиц-связок:
- множества, метки, корзины (bucket) — ключ = сущность, значения = элементы;
- вторичные индексы: ключ = индексное значение, значения = идентификаторы записей;
- временные ряды/сенсорные данные: ключ = идентификатор, значения = отсчёты;
- очередь по ключу: упорядоченные мультизначения дают естественный порядок выдачи.

API предоставляет подсчёт (`cursor_count`/`count_ex`), позиционирование по конкретному значению,
навигацию по первому/последнему значению ключа и выборочное добавление/удаление отдельных значений.

**Чем платим.** Вложенность добавляет уровень косвенности (и глубину), листовые узлы главного дерева
могут заполняться большим числом небольших узлов, а изменения на суб-страницах требуют CoW-копий —
при интенсивной записи это дороже плоской таблицы.

### 14.4. DBI-хендлы: жизненный цикл, состояние и большие количества

**Что такое хендл функционально.** `MDBX_dbi` — это число-индекс в пер-транзакционных массивах:
дескрипторов деревьев, курсоров, состояния и счётчиков поколений. Хендл — не глобальный объект, а
**условная привязка к таблице внутри конкретной транзакции**: он валиден в той транзакции, где его
открыли, и наследуется вложенными.

**Состояние.** Публично наблюдаемые биты состояния (`mdbx_dbi_flags_ex`):
- `DIRTY` — таблица изменялась в этой транзакции;
- `STALE` — кэшированный дескриптор таблицы старше номера транзакции;
- `FRESH` — хендл открыт в этой транзакции;
- `CREAT` — таблица создана в этой транзакции.

Внутри используются дополнительные пометки (`VALID`, `POISON`, `LINDO`, `OLDEN`, `SLAIN`) для
ленивой инициализации, распознавания закрытия/переоткрытия и пометки «удалена». Счётчик поколений
(`dbi_seqs`) на среду позволяет отличать «хендл, закрытый в другой транзакции» от «хендла никогда
не существовавшего»: повторное использование закрытого хендла даёт `MDBX_BAD_DBI`, а при наличии
«висячих» курсоров — `MDBX_DANGLING_DBI`. Распознавание ведётся по контрольным суммам сигнатур.

**Жизненный цикл.** open (при необходимости с `MDBX_CREATE`/`MDBX_DB_ACCEDE`) → использование →
close. Дескриптор, однажды созданный, остаётся в таблице таблиц; сам хендл можно открывать и закрывать
многократно в разных транзакциях. Внутренние служебные таблицы (главная и GC) имеют собственные
фиксированные номера.

**Связь с вложенными транзакциями.** Состояние хендла копируется из родителя (с очисткой
«свежих» битов), при коммите дочерние изменения дерева и состояния переносятся в родителя с
сохранением его статусов, при abort — состояние родителя восстанавливается (раздел 14.8).

**Огромное количество хендлов.** Лимит `MDBX_MAX_DBI = 32765` жёсткий, но практическая цена —
память пер-транзакционных массивов и размер главной таблицы. Методы работы при большом числе таблиц:
- **разреженная инициализация** — битовая карта «слот использован» избавляет от обнуления всего
  массива при старте каждой транзакции;
- открытие хендлов **пакетом** в одной транзакции (дороже один раз);
- **переиспользование хендлов** по схеме «закрыл → переоткрыл под новое имя» экономит слоты;
- осознанный компромисс: главная таблица с тысячами записей сама становится заметным B+tree,
  что сказывается на стоимости любых операций с таблицами.

### 14.5. Массовое удаление «гроздьями»

**Проблема.** Поочерёдное удаление N соседних ключей делает N операций CoW, каждая — со спуском по
дереву и копированием страниц. Для больших диапазонов это непропорционально дорого.

**Решение.** `mdbx_cursor_bunch_delete()` с режимом-действием (`MDBX_bunch_action_t`): удалить
текущее значение / всё текущее мультизначение / всё до или после позиции (включая/исключая) / всё
подряд. Вместо перебора элементов движок **вырезает целые страницы и ветки**, содержимое которых
подлежит удалению: дерево перестраивается на уровне «отрезать поддерево», а не «стереть записи».

**Что даёт.**
- стоимость пропорциональна **числу страниц/ветвей**, а не числу элементов;
- существенно меньше CoW-копий и записей в грязный список;
- освобождённые страницы поступают в GC крупными партиями (вплоть до BigFoot-цепочек);
- удобно для «устаревания» данных, очистки партиций, сброса сенсорных корзин.

**Сложности.** Отрезание ветки требует корректной перестройки родительских страниц (split/rebalance
в обратную сторону), позиционирования курсора после удаления и согласования с трекингом курсоров
(раздел 14.7). Для чтения массовыми партиями есть симметричный `mdbx_cursor_get_batch()`.

### 14.6. Оценка выборок по диапазонам

**Зачем.** Чтобы строить планы запросов, не выполняя их: «сколько элементов между ключом A и
ключом B?», «далеко ли следующий ключ?». Это функции `mdbx_estimate_range()`,
`mdbx_estimate_distance()`, `mdbx_estimate_move()`.

**Как работает.** Оценка строится по B+tree без прохода по данным: анализируются **общие страницы
стеков двух позиций** (высота дерева и заполненность узлов на общем пути). Расстояние оценивается
пропорционально плотности заполнения листьев в общей зоне. Открытые диапазоны кодируются
специальным маркером `MDBX_EPSILON`.

**Точность.** Оценка — именно оценка:
- в худшем случае (страницы заполнены от 25% до 100% из-за правил split/merge) результат может
  расходиться до **4× на каждом уровне** дерева, кроме первого и последнего;
- на практике крайние случаи редки, типичная ошибка — единицы процентов;
- точность зависит от истории вставок/удалений и разброса длин ключей.

**Применение.** Планировщики запросов (выбор индекса, оценка селективности), расчёт размера
страниц пагинации, предварительная оценка объёма результатов, балансировка диапазонных задач.

### 14.7. Отслеживание изменений и состояние курсоров в пишущей транзакции

**Проблема.** Курсор хранит стек позиций (страница, индекс) прямо в mmap. В пишущей транзакции любая
CoW-операция создаёт новые версии страниц: если курсор продолжит ссылаться на старую версию, он либо
«застынет» на устаревших данных, либо получит указатель на переиспользованную страницу.

**Механизм.** Каждая пишущая транзакция ведёт учёт курсоров:
- на страницах-версиях хранятся сигнатуры; курсоры перепроверяются при операциях, способных сдвинуть
  данные (put/del/split/merge);
- при «перекладывании» данных на новую версию страницы стек курсора **перенацеливается**
  (repoint) на новую версию, либо позиция помечается для ленивого перепозиционирования;
- для вложенных транзакций применяется паттерн **«спаренных» курсоров** (shadow/couple): курсоры
  родителя проецируются в дочернюю транзакцию, а при её завершении — обратно;
- отладочные зонды проверяют стеки всех курсоров на «висячие» (dangling) ссылки после изменений.

**Что даёт.** Курсоры остаются валидными во время записи: итерация «читаем-правим-удаляем» в одной
транзакции корректна и безопасна. Прикладной код может дополнительно использовать `mdbx_is_dirty()`,
чтобы не копировать данные с грязных страниц, которые будут перезаписаны.

**Чем платим.** Каждая операция записи несёт небольшую нагрузку на поддержание стека; пометка «сброса»
курсора оформлена одним записываемым значением, чтобы сделать переключение состояний максимально
дешёвым.

### 14.8. Вложенные транзакции: реализация и судьба таблиц

**Реализация.** Вложенная (nested) транзакция — это дочерняя пишущая транзакция внутри родительской,
на том же номере транзакции и предварительном `front txnid`. Она наследует у родителя:
- пространство страниц и грязный список (получает собственную копию списка с тенью родительского);
- retired-страницы, спилл-список, буферы GC (rkl/pnl) — с возможностью слияния назад;
- дескрипторы таблиц и состояния хендлов (копируются с очисткой «свежих» битов);
- курсоры — через механизм «спаренных» курсоров (раздел 14.7).

Флаг `MDBX_TXN_HAS_CHILD` маркирует родителя.

**Коммит (join).** Изменения дочерней транзакции **вплавляются в родителя**: грязные страницы
сливаются в родительский список (объединение грязных списков с учётом перекрытий), loose/refund —
передаются, счётчики GC переносятся. Если дочерняя транзакция была «чистой» (ничего не меняла),
выполняется быстрый путь без слияния страниц. Курсоры родителя переводятся на новые версии страниц.

**Abort (undo).** Все изменения дочерней транзакции **отбрасываются**: грязные страницы возвращаются
родителю в исходном виде, утилизированные страницы «рас-утилизируются», состояние хендлов и курсоры
восстанавливаются.

**Судьба таблиц.** Поскольку создание/удаление таблицы — это изменение главной таблицы (CoW), оно
подчиняется тем же правилам:
- таблица, **созданная** во вложенной транзакции: при коммите — остаётся (запись в главной таблице
  попадает в родителя); при abort — исчезает, хендл теряет бит `CREAT` и становится обычным;
- таблица, **удалённая** во вложенной транзакции: при коммите — удаляется (в родителе помечается
  как `SLAIN`); при abort — «оживает»: состояние хендла восстанавливается, дерево на месте.

Итог: вложенные транзакции дают «под-откат» для групп операций, включая операции с таблицами.

### 14.9. Парковка и вытеснение транзакций

**Проблема.** Читательская транзакция, живущая «по-серверному» (коннектор, обработчик событий),
пинит детент и блокирует переиспользование (раздел 17.2). Убивать её нельзя — данные нужны.

**Решение — парковка.** `mdbx_txn_park()` переводит читателя в состояние `MDBX_TXN_PARKED`: слот в
таблице читателей помечается специальным псевдо-идентификатором и **перестаёт влиять на детент**.
Снапшот больше не удерживается, поэтому GC может продвигаться. Возобновление (`mdbx_txn_unpark()`)
«оживляет» слот тем же потоком — и, если вытеснения не было, продолжение стоит дёшево (не нужен полный
перезапуск). Опция `autounpark` включает прозрачное возобновление при первом же вызове API чтения.

**Вытеснение (ousting).** Если писателю не хватает пространства, он «выбивает» припаркованные
читатели: слот атомарно переводится из `PARKED` в `OUSTED` (CAS), снапшот становится переиспользуемым.
При следующем обращении вытесненная транзакция узнаёт об этом:
- с `restart_if_ousted=true` — немедленно перезапускается на свежем снапшоте (аналог renew);
- иначе — сбрасывается и возвращает `MDBX_OUSTED`, после чего её можно возобновить явно.

Флаг `MDBX_TXN_OUSTED` виден через `txn_flags()`; парковка является штатной «полкой» для HSR-сценариев
(раздел 9.2): сначала выселяются припаркованные, затем вызывается колбэк для остальных.

**Что даёт.** Долгоживущие логические чтения сосуществуют с активной записью, не морозя GC; возобновление
при отсутствии давления обходится минимально; выселение гарантирует, что память в итоге освободится.

**Важное предостережение.** После парковки снапшот не удерживается: разыменовывать указатели на данные,
полученные до парковки, запрещено — страницы могут быть переиспользованы в любой момент.

### 14.10. Управление размером базы «на ходу»

**Рост.** Когда GC исчерпан, писатель выделяет страницы из неразмеченного хвоста файла (раздел 10).
Расширение файла выполняется «на лету»:
- шаг роста кратен `growth_step` геометрии (раздел 2.3), новые страницы не трогаются ядром до первого
  обращения (без page-fault на всю область);
- другие процессы узнают об увеличении прозрачно при старте следующей транзакции; если расширить
  адресное пространство невозможно — возвращается `MDBX_UNABLE_EXTEND_MAPSIZE`, и требуется
  переоткрыть окружение.

**Сжатие.** Обратный процесс — укорочение файла — выполняется только когда «хвост» свободен и никем
не виден (раздел 14.11), причём фактически отрезается край файла, а промежуток обрабатывается
`madvise(MADV_DONTNEED/MADV_REMOVE)`.

**Сложности.**
- **Windows**: файл нельзя сжать, не сняв mmap целиком (недопустимо при нескольких процессах);
  расширение возможно только через «Native API»; при изменении геометрии приходится приостанавливать
  все потоки процесса; требуется дополнительный SRWL на каждую читательскую транзакцию; сжатие
  доступно только в однопроцессном сценарии.
- **Гонка верхней границы**: увеличивать `upper` на лету надёжно нельзя (адресное пространство —
  общий и изменчивый ресурс), поэтому верхнюю границу задают заранее.
- **Флаунсинг**: шаг сжатия должен превышать шаг роста, иначе файл будет «дышать» вверх-вниз
  (гистерезис).

### 14.11. Бесплатная авто-компактификация

**Что это.** «Бесплатная» компактификация — это **неявное сжатие** (implicit shrink), которое
происходит само, в составе обычных коммитов, без отдельного прохода по данным. Оно «бесплатно» в том
смысле, что не требует остановки приложения, отдельной команды и не читает данные.

**Механика.** Освобождённые хвостовые страницы возвращаются в неразмеченное пространство через refund
(раздел 5.3) при каждой фиксации записи. Когда «свободный хвост» превышает порог сжатия
(`shrink_threshold`) плюс небольшой запас (`stockpile_gap ≈ 3 + высота_GC×3`), а край файла никто не
держит (нет самого старого снапшота за границей), коммит дополнительно:
1. сбрасывает «лишний» диапазон через `madvise(MADV_DONTNEED/REMOVE)`;
2. усекает файл до нижней границы с выравниванием по системному размеру блока.

**Что даёт.** Размер файла остаётся ограниченным реальными данными (а не историей churn), пространство
возвращается операционной системе, а локальность размещения не деградирует со временем. Всё это — без
отдельного обслуживания.

**Ограничения.**
- Не работает, пока долгие читатели удерживают хвост (раздел 17.2) или файл используют несколько
  процессов (на Windows — строго один процесс);
- при `MDBX_SAFE_NOSYNC` сжатие ограничено частотой steady-point'ов;
- гистерезис (порог сжатия > шага роста) обязателен, чтобы избежать «дыхания» файла.

### 14.12. Дефрагментация: как работает и что даёт

**Цель.** Вернуть базе компактность: страницы из конца файла переносятся на свободные страницы ближе к
началу, после чего освободившийся хвост отрезается. Это не «чтение-перезапись», а **аккуратная
перестановка** с минимумом записи на диск.

**Механика.**
1. Строится карта связей страниц (список дуг/циклов перемещения).
2. Живые страницы перемещаются по циклам к началу файла; родительские страницы корректируются
   (CoW-копии вдоль пути к корню — тот же механизм, что и при обычной записи).
3. Для переноса overflow-прогонов нужны соседние свободные последовательности — иногда приходится
   двигать дополнительные страницы.
4. Так как перенос родительских страниц сам расходует свободные страницы, полная дефрагментация почти
   никогда не укладывается в один проход: она выполняется **циклами**, каждый из которых — отдельная
   коммитнутая транзакция, прерываемая в любой момент без потери результата.
5. Параллельные читатели не мешают, но ограничивают дефрагментацию одним циклом (полноте завершения
   мешают).

**Параметры.** `defrag_atleast`/`defrag_enough` (минимальное и достаточное сокращение в страницах),
`time_atleast`/`time_limit` (в 1/65536 секунды), `acceptable_backlash` (остановка, если следующий цикл
даст меньше указанного), `preferred_batch` (целевой размер цикла в страницах), колбэк прогресса.
Результаты отдаются структурой с кодами, включая `MDBX_LAGGARD_READER` (мешают долгие читатели) и
«цели не достигнуты из-за лимитов».

**Что даёт.**
- возврат занятого «мусором» пространства и фактическое уменьшение файла;
- улучшение локальности и плотности размещения (быстрее последовательные чтения);
- подготовку базы к резервному копированию и миграции;
- работает онлайн, ACID-безопасно по циклам.

**Чем платим.** Запись при перемещении (страница пишется дважды: на новое место и через CoW-путь),
необходимость циклов из-за расхода свободных страниц на копии родителей и сложность согласования с GC,
refund и читателями.
...
### 15.1. Создание базы и первый коммит
### 15.2. Типичное чтение
### 15.3. Интенсивный write-churn
### 15.4. Открытие после сбоя
### 15.5. Резервное копирование / проверка / дефрагментация
...
| **FIFO/LIFO (reclaiming)** | Политика переиспользования освобождённых страниц: по умолчанию FIFO (старые раньше), `MDBX_LIFORECLAIM` — LIFO (свежие раньше) |
| **Dupsort-таблица** | Таблица-мультикарта: один ключ → упорядоченное множество значений (вложенное дерево/суб-страницы) |
| **Bunch delete** | Массовое удаление «гроздьями»: вырезание целых страниц/ветвей вместо поштучного удаления |
| **Range estimation** | Эвристическая оценка расстояния/числа элементов в диапазоне по общим страницам стеков курсоров |
| **Парковка / вытеснение** | Освобождение читателем слота с сохранением хендла (park); принудительный «выселение» снапшота (ousted) |
| **Implicit shrink** | Бесплатная авто-компактификация: усечение свободного хвоста файла в составе обычных коммитов |
| **Трим/Sync modes** | Режимы долговечности (durable/nometasync/safe_nosync/utterly_nosync/writemap) |

---

## 17. Темы и вопросы читателей: развёрнутые ответы

> Этот раздел собран из тем, которые чаще всего интересуют читателей документа. Часть из них подробно
> разобрана в разделе 14 («что это даёт», «как работает»), а здесь — сводные развёрнутые ответы на
> сквозные вопросы. Ответы строятся на механизмах, описанных в разделах 2–14, с указанием
> соответствующих разделов.

---

### 17.1. Почему помещение в БД больших значений может приводить к глубокому поиску и переработке GC?
...
### 17.2. Почему и когда долгие читающие транзакции могут приводить к росту и переполнению БД? Краткосрочные и долгосрочные перспективы.
...
### 17.3. Как работает кеширование с ленивым поиском и почему оно может ускорять производительность в тысячи раз?
...
### 17.4. Как WAF определяется MVCC-подходом libmdbx, какова зависимость WAF от объёма данных и почему?
...
- **Косвенное влияние объёма через GC и локальность.** Чем больше база:
  - больше записей в GC-дереве, его обслуживание глубже и дороже (разделы 6, 17.1);
...
- **Эффект долгих читателей.** Заморозка переиспользования (раздел 17.2) заставляет выделять новые
