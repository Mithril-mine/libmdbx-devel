...
## Event-driven loop («медитация»)

- Штатный режим работы координатора: команды владельца «медитируй» /
  «помедитируй» = переход в event-driven loop.
- Подробности: `skills/orchestrator/meditation/SKILL.md`.
- Кратко: вызывать `.skynet/coordinator-step.sh 0.coordinator` (блокирующий
  waitmail, rc 0=MAIL / 130=OWNER-CALL / 1=NO-MAIL), обрабатывать событие,
  снова вызывать утилиту — НЕ ждать промпт.
