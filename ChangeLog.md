...
### Fixes:
 ...
 - Fixed `mdbx_test --duration`: the wall-clock budget was applied inconsistently across actors — fast scenarios stopped early on the implicit `--nops=1000` bound, while hill's downhill phase and the nested/ttl whole-table drains overran the deadline by several times. `--duration` is now a total budget across all `--repeat` iterations and dominates the implicit nops safety bound; actors stop promptly at the next safe point, and the internal `--timeout` watchdog now covers the setup/barrier phase as well.
